...
@WebServlet(name = "PriorityListCtl", urlPatterns = { "/ctl/PriorityListCtl" })
public class PriorityListCtl extends BaseCtl{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<PriorityBean> list = model.search(bean, pageNo, pageSize);
			List<PriorityBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					PriorityBean deletebean = new PriorityBean();
					for (String id : ids) {
	...
	protected String getView() {
		...
		return ORSView.PRIORITY_LIST_VIEW;
	}
